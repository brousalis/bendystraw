...
function inject(callback) {
  ...
  var injectStyles = gulp.src(path.join(config.paths.dev, config.paths.styles, '/**/*.css'), { read: false });

  var injectStylesOptions = {
    ignorePath: [config.paths.src, path.join(config.paths.dev)],
    addRootSlash: false
  };
  ...
  if (scripts.length > 0) {
    scripts.forEach(function(path) {
      ...
      if (/^\!/.test(path)) {
        paths.push(path.substr(0, 1) + config.paths.dev + '/' + path.substr(1))
      } else {
        paths.push(config.paths.dev + '/' + path)
      }
    });
  } else {
    paths = [
      path.join(config.paths.dev, config.paths.scripts, '/**/*.js'),
      path.join('!' + config.paths.dev, config.paths.scripts, '/**/*.spec.js'),
      path.join('!' + config.paths.dev, config.paths.scripts, '/**/*.mock.js'),
    ]
  }
  ...
  var injectScriptsOptions = {
    ignorePath: [config.paths.src, config.paths.dev],
    addRootSlash: false
  };

  // Inject file containing environment settings, just incase
  // this file needs to be imported before others
  var injectEnv = gulp.src(path.join(config.paths.dev, config.paths.scripts, '/env.js'), { read: false });
  ...
  var injectVendor = gulp.src(path.join(config.paths.dev, config.paths.vendor, '/**/*.js'), { read: false });
  ...
  var injectTemplates = gulp.src(path.join(config.paths.dev, '/templates/templates.js'), { read: false });
